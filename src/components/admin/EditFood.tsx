...
export default function EditFood() {
  ...
  useEffect(() => {
    if (foodInfo?.result) {
      const fetchData = foodInfo?.result?.food;
      setFormData({
        name: fetchData?.name,
        foodType: fetchData?.foodType,
        dietPreferences: fetchData?.dietPreferences,
        price: fetchData?.price,
        discount: fetchData?.discount,
        description: fetchData?.description,
        restaurantsIds: formData.restaurantsIds,
      });
    }
  }, [foodInfo?.result]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    imageFile && form.append("foodImage", imageFile);
    formData.restaurantsIds.forEach((id) => {
      form.append("restaurantsIds[]", id);
    });

    try {
      const response: any = await updateFoodFunc({ foodId, form });
